...
public abstract class SingleSecurityTest {
    ...
    public SingleSecurityTest withProviderSettings(final int providerTimeframe, final TimeUnit providerTimeUnit, final String... paths) {
     ...
     */
    public SingleSecurityTest loadMarketData(final String secPrefix, final int from, final int till, final LongPredicate timeFilter) throws IOException {
        this.secPrefix = secPrefix;
        marketDataMap.clear();
        loadMarketData(from, till, timeFilter);
        return this;
    }
    ...
    private void loadMarketData(final int from, final int till, final LongPredicate timeFilter) throws IOException {
        if (!enableFuturesPrefix || Futures.withPrefix(secPrefix).length == 0) {
            loadStockData(from, till, timeFilter);
        } else {
            loadFuturesData(from, till, timeFilter);
        }
    }
    ...
    private void loadFuturesData(final int from, final int till, final LongPredicate timeFilter) throws IOException {
